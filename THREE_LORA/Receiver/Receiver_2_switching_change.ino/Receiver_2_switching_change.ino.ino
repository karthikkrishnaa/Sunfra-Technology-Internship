...
void SMITdataAPI() {
  ...
  client.setInsecure();
  HTTPClient http;
  String url = String(SMIT_API) + "?mac_address=" + g_deviceMAC + "&value=" + g_smitweightValue;
...
void handleReset() {
  ...
  html += "<p>Old network deleted. Connecting to new network...</p></body></html>";
  ...
  // Determine which saved network we're switching TO
  ...
  Serial.println("[Reset] Deleting old WiFi, keeping: " + targetSSID);

  // ⬅ NEW: wipe old credentials, shift the new one into slot 1
  shiftToSingleWiFi(targetSSID, targetPassword);

  currentWiFiSlot = 1;  // we always end up in slot 1 now
  portalClosed = false; // ⬅ NEW: allow the portal to reopen, since count is now 1
  ...
  // ⬅ NEW: reopen the config portal so user can add a replacement second WiFi
  wm.setConfigPortalBlocking(false);
  wm.startWebPortal();
  Serial.println("[Reset] Config portal reopened for adding new WiFi");
}

void shiftToSingleWiFi(String keepSSID, String keepPassword) {
  prefs.begin("wifi", false);

  // Wipe both slots first
  prefs.remove("ssid1");
  prefs.remove("pass1");
  prefs.remove("ssid2");
  prefs.remove("pass2");

  // Write the "keep" network into slot 1
  prefs.putString("ssid1", keepSSID);
  prefs.putString("pass1", keepPassword);

  prefs.end();

  Serial.println("[Prefs] Shifted to single WiFi: " + keepSSID);

  // Refresh in-memory state immediately
  loadSavedWiFiNetworks();   // this will now show savedWiFiCount == 1
}
// ─────────────────────────────────────────────────────
// Setup
// ─────────────────────────────────────────────────────
...
if (portalClosed && !serverStarted && savedWiFiCount >= 2) {
  ...
  if (packetSize) {
    ...
    g_smitweightValue = getValue(received, "WT:");
    ...
    Serial.println("MAC         : " + g_deviceMAC);
    Serial.println("Packet No   : " + g_pktNumber);

    if (g_deviceID == "FARM01") {
    ...
    if (g_deviceID == "FARM02") {
       Serial.println("SMIT weight Value  : " + g_smitweightValue);
    }
